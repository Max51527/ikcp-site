...
export default {
  async fetch(request, env) {
    ...
    // ── CODEX SOUVERAIN — Mistral Large (FR) si LLM_PRIMARY=mistral (réversible) ──
    // Échec Mistral → fallback Anthropic ci-dessous. Données déjà souveraines.
    if (env.LLM_PRIMARY === 'mistral' && env.MISTRAL_API_KEY) {
      try {
        ...
        if (mr.ok) {
          ...
          if (reply) return Response.json({ reply, agent: 'Codex', model: 'ikcp-souverain', delegated_by: context ? 'Marcel' : 'direct' }, { headers: corsHeaders(origin) });
        }
